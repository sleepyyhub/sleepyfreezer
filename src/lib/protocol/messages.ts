...
export const helloMessageSchema = z.object({
  ...
  /**
   * Stable per-executor identity, generated by the client and kept in getgenv so
   * it survives re-running the script. Re-running in the same executor reuses the
   * key and replaces that connection; a second person on another machine brings a
   * different key and joins as an additional client instead of evicting the first.
   */
...
export interface HelloAckMessage {
  ...
  type: 'hello_ack';
  sessionId: string;
  /** Server-assigned handle for this client, used to address it in tool calls. */
  clientId: string;
  /** Human-readable label the dashboard and agent use for this client. */
  clientLabel: string;
  ...
  serverTime: number;
  /** When false the bridge prints nothing to the executor console. */
  logging: boolean;
}

/** Pushed when the owner changes a session setting that the client honours. */
...
export const ErrorCodes = {
  ...
  /** More than one Roblox client is connected and the call named none of them. */
