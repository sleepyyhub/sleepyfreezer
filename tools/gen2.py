CELLSEQ=[0]
import pickle,clov,lin,dec,sys
    ...
    def emitblock(self,a,b,ind,out):
        ...
        while i<b:
            ...
            if n=='MOVE':
                ...
                ch=self.p.children[C-1]; desc=ch.t[12]; ups=[]
                for x in range(len(desc)//2):
                    aa=clov.dec32(desc[2*x],2*x+1,ch.kA,ch.kB,0)
                    bb=clov.dec32(desc[2*x+1],2*x+2,ch.kA,ch.kB,0)
